...
// ── Protobuf wire format helpers for InnerTube get_transcript params ──
...
function encodeString(fieldNumber: number, value: string): number[] {
  ...
  return [...encodeVarint((fieldNumber << 3) | 2), ...encodeVarint(encoded.length), ...encoded];
}

function encodeBool(fieldNumber: number, value: boolean): number[] {
  return [...encodeVarint((fieldNumber << 3) | 0), value ? 1 : 0];
}

function encodeMessage(fieldNumber: number, content: number[]): number[] {
  return [...encodeVarint((fieldNumber << 3) | 2), ...encodeVarint(content.length), ...content];
}

/**
 * Build base64-encoded protobuf `params` for InnerTube get_transcript.
 *
 ...
 *     }
 *     field 2 (message) {
 *       field 1 (string): source ("asr" for auto-generated, "" for manual)
 *       field 2 (string): langCode
 *       field 4 (bool):   true
 *     }
 *   }
 */
function buildTranscriptParams(
  ...
  const videoIdMsg = encodeMessage(1, encodeString(1, videoId));
  const langMsg = encodeMessage(2, [
    ...encodeString(1, isAutoGenerated ? "asr" : ""),
    ...encodeString(2, langCode),
    ...encodeString(3, ""), // language name (empty)
    ...encodeBool(4, true),
  ]);
  const inner = encodeMessage(1, [...videoIdMsg, ...langMsg]);
  const b64 = btoa(String.fromCharCode(...inner));
  // URL-encode the base64 (matches youtube-transcript-api behavior)
  return encodeURIComponent(b64);
}

/** YouTube InnerTube API key (public, embedded in youtube.com web client) */
const INNERTUBE_API_KEY = "AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8";

/**
 * Fetch visitorData from InnerTube /player (needed for get_transcript auth).
 * Uses IOS client which reliably works from Vercel IPs (WEB client returns restricted responses).
 * Returns visitorData string or undefined if unavailable.
 */
async function fetchVisitorData(videoId: string): Promise<string | undefined> {
  try {
    const iosClient = INNERTUBE_CLIENTS[0]; // IOS (reliable from Vercel)
    const res = await fetchWithTimeout(
      `https://www.youtube.com/youtubei/v1/player?key=${INNERTUBE_API_KEY}`,
      {
        ...
        headers: {
          "Content-Type": "application/json",
          "User-Agent": iosClient.userAgent,
        },
        body: JSON.stringify({ videoId, context: iosClient.context }),
      },
      3000,
    );
    if (!res.ok) return undefined;
    const data = await res.json();
    return data?.responseContext?.visitorData ?? undefined;
  } catch {
    return undefined;
  }
}
...
/**
 * Fetch caption cues via InnerTube get_transcript endpoint.
 * Unlike timedtext URLs, this returns transcript text directly in the response body,
 * bypassing POT/IP blocks that affect timedtext baseUrl requests on Vercel.
 * Requires WEB client context with visitorData + API key for authentication.
 */
async function fetchCuesFromTranscript(
  videoId: string,
  lang: string,
  isAutoGenerated: boolean,
): Promise<CaptionCueRaw[] | null> {
  const params = buildTranscriptParams(videoId, lang, isAutoGenerated);

  // get_transcript requires visitorData for authentication
  const visitorData = await fetchVisitorData(videoId);
  if (!visitorData) {
    console.warn("[captions] get_transcript: failed to obtain visitorData, skipping");
    return null;
  }

  const webClient = INNERTUBE_CLIENTS[2]; // WEB client (transcript is a web UI feature)
  const context = {
    client: {
      ...webClient.context.client,
      visitorData,
    },
  };
  ...
  try {
    const res = await fetchWithTimeout(
      `https://www.youtube.com/youtubei/v1/get_transcript?key=${INNERTUBE_API_KEY}`,
      {
        ...
        headers: {
          ...
          "X-YouTube-Client-Version": webClient.context.client.clientVersion,
        },
    ...
    if (!res.ok) {
      console.warn(`[captions] get_transcript: HTTP ${res.status}`);
      return null;
    }

    const data = await res.json();

    // Navigate response: actions[0].updateEngagementPanelAction.content
    ...
    if (!Array.isArray(cueGroups) || cueGroups.length === 0) {
      console.warn("[captions] get_transcript: no cueGroups in response");
      return null;
    }

    const cues: CaptionCueRaw[] = [];
    for (const group of cueGroups) {
      const cueData = group?.transcriptCueGroupRenderer?.cues?.[0]?.transcriptCueRenderer;
      if (!cueData) continue;

      const text = (cueData.cue?.simpleText as string | undefined)?.replace(/\n/g, " ").trim();
      if (!text) continue;

      const startMs = Number(cueData.startOffsetMs ?? 0);
      const durationMs = Number(cueData.durationMs ?? 0);
      const start = startMs / 1000;
      const duration = durationMs / 1000;

      cues.push({ start, duration, end: start + duration, text });
    }

    if (cues.length > 0) {
      console.log(`[captions] get_transcript: got ${cues.length} cue(s) for ${videoId}/${lang}`);
      return cues;
    }

    console.warn("[captions] get_transcript: parsed 0 cues");
  } catch (err) {
    console.warn(`[captions] get_transcript failed: ${err instanceof Error ? err.message : err}`);
  }

  return null;
}

/** Fetch and parse caption cues from a baseUrl, with get_transcript / Piped / Invidious fallback */
async function fetchCaptionCues(
  ...
  // Step 2: InnerTube get_transcript (returns cues directly, bypasses timedtext IP blocks)
  const transcriptCues = await fetchCuesFromTranscript(videoId, lang, isAutoGenerated);
  if (transcriptCues && transcriptCues.length > 0) return transcriptCues;

  // Step 3: try baseUrl as-is without fmt=json3 (Invidious returns WebVTT directly)
  try {
  ...
  throw new Error(
    "Failed to fetch caption cues from timedtext, get_transcript, Piped, and Invidious",
  );
}
...
async function runDebugDiagnostics(videoId: string, lang?: string) {
  ...
  if (lang) {
    ...
    // Find matching track's baseUrl and auto-generated flag
    let trackBaseUrl: string | null = null;
    let trackIsAuto = false;
    try {
      ...
      cueFetch.trackIsAutoGenerated = track?.isAutoGenerated ?? null;
      cueFetch.allTrackLangs = meta.captionTracks.map((t) => t.languageCode);
      trackBaseUrl = track?.baseUrl ?? null;
      trackIsAuto = track?.isAutoGenerated ?? false;
    } catch (err) {
    ...
    // Step 2.5: InnerTube get_transcript (with visitorData + API key)
    let debugVisitorData: string | undefined;
    try {
      debugVisitorData = await fetchVisitorData(videoId);
      cueFetch.step2_5_visitorData = debugVisitorData
        ? `${debugVisitorData.slice(0, 20)}...`
        : null;
    } catch (err) {
      cueFetch.step2_5_visitorData = {
        error: err instanceof Error ? err.message : String(err),
      };
    }

    for (const variant of [
      { key: "transcript_asr", isAuto: true },
      { key: "transcript_manual", isAuto: false },
    ]) {
      const transcriptParams = buildTranscriptParams(videoId, lang, variant.isAuto);
      const webClient = INNERTUBE_CLIENTS[2];
      const stepKey = `step2_5_${variant.key}`;
      const context = {
        client: {
          ...webClient.context.client,
          ...(debugVisitorData && { visitorData: debugVisitorData }),
        },
      };
      try {
        const res = await fetchWithTimeout(
        ...
        const cueGroups =
          parsed?.actions?.[0]?.updateEngagementPanelAction?.content?.transcriptRenderer?.body
            ?.transcriptBodyRenderer?.cueGroups;
        cueFetch[stepKey] = {
          status: res.status,
          contentLength: text.length,
          ...
          firstActionType: parsed?.actions?.[0] ? Object.keys(parsed.actions[0]).join(",") : null,
          cueGroupCount: Array.isArray(cueGroups) ? cueGroups.length : 0,
          firstCuePreview:
            Array.isArray(cueGroups) && cueGroups.length > 0
              ? (cueGroups[0]?.transcriptCueGroupRenderer?.cues?.[0]?.transcriptCueRenderer?.cue
                  ?.simpleText ?? null)
              : null,
          matchesTrack: variant.isAuto === trackIsAuto,
        };
      } catch (err) {
        cueFetch[stepKey] = {
          error: err instanceof Error ? err.message : String(err),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rawCues = await fetchCaptionCues(track.baseUrl, videoId, lang, track.isAutoGenerated);
