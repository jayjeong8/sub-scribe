...
async function fetchCuesFromTranscript(
  ...
  const clients = INNERTUBE_CLIENTS.slice(0, 2); // IOS, ANDROID only
...
async function runDebugDiagnostics(videoId: string, lang?: string) {
  ...
  if (lang) {
    ...
    // Step 2.5: InnerTube get_transcript (both ASR and manual)
    for (const variant of [
      { key: "step2_5_transcript_asr", isAuto: true },
      { key: "step2_5_transcript_manual", isAuto: false },
    ]) {
      try {
        const cues = await fetchCuesFromTranscript(videoId, lang, variant.isAuto);
        cueFetch[variant.key] = {
          success: cues !== null && cues.length > 0,
          cueCount: cues?.length ?? 0,
          preview: cues?.slice(0, 2).map((c) => ({ start: c.start, text: c.text })) ?? [],
