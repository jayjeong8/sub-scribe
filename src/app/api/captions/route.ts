...
/** Fetch caption tracks from Piped API */
  ...
  for (const instance of PIPED_INSTANCES) {
    try {
      ...
      const subtitles: { url?: string; mimeType?: string; name?: string; code?: string }[] =
        data.subtitles;
      if (!Array.isArray(subtitles) || subtitles.length === 0) continue;
      ...
      if (captionTracks.length === 0) continue;
      ...
      console.warn(
        `[captions] piped (${instance}) failed for ${videoId}: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  return null;
...
/** Fetch caption cues from Piped API (fetches /streams then subtitle URL) */
async function fetchCuesFromPiped(videoId: string, lang: string): Promise<CaptionCueRaw[] | null> {
  for (const instance of PIPED_INSTANCES) {
    try {
      ...
      console.warn(
        `[captions] piped cues (${instance}) failed: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  return null;
