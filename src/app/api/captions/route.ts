...
async function fetchCuesFromTranscript(
  ...
  // WEB client first (transcript panel is a web UI feature), then IOS/ANDROID fallback
  const clients = [INNERTUBE_CLIENTS[2], INNERTUBE_CLIENTS[0], INNERTUBE_CLIENTS[1]];
  ...
  for (const client of clients) {
    try {
      ...
      if (!res.ok) {
        console.warn(`[captions] get_transcript (${client.label}): HTTP ${res.status}`);
        continue;
      }
      ...
      if (!Array.isArray(cueGroups) || cueGroups.length === 0) {
        console.warn(`[captions] get_transcript (${client.label}): no cueGroups in response`);
        continue;
      }
      ...
      if (cues.length > 0) {
        console.log(
          `[captions] get_transcript (${client.label}): got ${cues.length} cue(s) for ${videoId}/${lang}`,
        );
        return cues;
      }

      console.warn(`[captions] get_transcript (${client.label}): parsed 0 cues`);
    } catch (err) {
      console.warn(
        `[captions] get_transcript (${client.label}) failed: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
...
async function runDebugDiagnostics(videoId: string, lang?: string) {
  ...
  if (lang) {
    ...
    // Step 2.5: InnerTube get_transcript (raw request per client + variant)
    for (const variant of [
      { key: "transcript_asr", isAuto: true },
      { key: "transcript_manual", isAuto: false },
    ]) {
      const params = buildTranscriptParams(videoId, lang, variant.isAuto);
      // Test WEB client (transcript is a web UI feature)
      const webClient = INNERTUBE_CLIENTS[2];
      const stepKey = `step2_5_${variant.key}_WEB`;
      try {
        const res = await fetchWithTimeout(
          "https://www.youtube.com/youtubei/v1/get_transcript",
          {
            ...
            body: JSON.stringify({ context: webClient.context, params }),
          },
          4000,
        );
        const text = await res.text();
